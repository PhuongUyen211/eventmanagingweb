...
class SignController {
    ...
    async register(req, res) {
        try {
            ...
            upload.single('avatar')(req, res, async (err) => {
                ...
                if (existingUser) {
                    return res.status(400).json({ message: 'Email người sử dụng đã tồn tại!' });
                }
    ...
    async login(req, res) {
        try {
            ...
            if (!email || !password) {
                res.status(400).json({ message: 'Email và mật khẩu không thể để trống!' });
            }
            ...
            let existingUser = await user.findOne({ email: email });
            if (!existingUser) {
                res.status(400).json({ message: 'Không tồn tại email người dùng!' });
            }
    ...
    async forgotPw(req,res){
        ...
        try{
            ...
            const emailResponse = await sendResetPasswordEmail(User.email, resetToken);
            if (!emailResponse.success) {
                res.status(500).json({ message: 'Đã gửi email với mã đặt lại mật khẩu', response: emailResponse.response });
            }

            return res.json({ message: 'Đã gửi email với mã đặt lại mật khẩu', response: emailResponse.response });
